...
namespace BLL
{
    public class clAcessosBll
    {
        ...
        public List<TOAcessosBll> Listar()
        {
            ...
            string _sql = @"
                SELECT  Pessoas.IdPessoa, Pessoas.Nome, Pessoas.Usuario, 
                        AcessosAuxFuncoes.IdAcFunc, AcessosAuxFuncoes.Funcao, 
                        AcessosVigenciaPlanos.IdVigencia, AcessosVigenciaPlanos.DtInicial, 
                        AcessosVigenciaPlanos.DtFinal, 
                        AcessosVigenciaPlanos.ComprovanteHomologado, 
						...
						END) As Situacao,
                        PlanosAssinaturas.QtdAnim, 
                        (CASE PlanosAssinaturas.Periodo  
							WHEN 1 THEN 'Mensal'
							WHEN 2 THEN 'Anual'
						END) As Periodo, 
                        PlanosAssinaturas.Receituario, PlanosAssinaturas.Prontuario, 
                ...
                ORDER BY    AcessosAuxFuncoes.Funcao, Pessoas.Usuario ";
        ...
        public TOSistemaDWHBll ResumoSistema(int _idPessoa)
        {
            ...
            string _sql = string.Format(@"
                SELECT	
                    ...
                    (Select	Count(p.IdPessoa) 
                     From	Pessoas p
                     Where	(p.Ativo = 1) AND (p.IdCliente = {0})) TotalTutores,
                    (SELECT	Count(an.IdAnimal) 
                     FROM	Pessoas AS p INNER JOIN
			                    Animais AS an ON p.IdPessoa = an.IdPessoa
                     WHERE	(an.Ativo = 1) AND 
		                    (an.IdPessoa IN (SELECT	IdPessoa
						                     FROM	Pessoas AS p
						                     WHERE	(IdCliente = {0})))) TotalPacientes", 
               _idPessoa);
