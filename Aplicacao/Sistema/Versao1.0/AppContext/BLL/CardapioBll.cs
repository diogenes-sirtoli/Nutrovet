...
namespace BLL
{
    ...
    public class clCardapioBll
    {
        ...
        public TOCardapioBll CarregarTO(int _idCardapio)
        {
            ...
            string _sql = string.Format(@"
                ...
                SELECT  c.IdPessoa AS IdTutor, p.Nome AS Tutor, c.IdAnimal, 
                        a.Nome AS Animal, e.Especie, r.Raca, c.IdCardapio, c.Descricao, 
                        c.DtCardapio, a.PesoAtual, a.PesoIdeal, 
                        (CASE a.Sexo 
                            WHEN 1 THEN 'Macho' 
                        ...
                        DATEDIFF(YY, a.DtNascim, GETDATE()) - 
                            CASE WHEN RIGHT(CONVERT(VARCHAR(6), GETDATE(), 12), 4) >= 
                                ...
                                THEN 0 
                                ELSE 1 END AS Idade, 
                        d.Dieta, c.FatorEnergia, 
                        c.NEM, c.Lactante, c.Gestante, c.LactacaoSemanas, c.NrFilhotes, 
                        c.IdDieta, c.EmCardapio, c.Observacao, c.Ativo, c.IdOperador, 
                        c.IP, c.DataCadastro
                FROM    Pessoas AS p INNER JOIN
                            Animais AS a ON p.IdPessoa = a.IdPessoa RIGHT OUTER JOIN
        ...
        public List<TOCardapioBll> Listar(int _idAnimal)
        {
            ...
            string _sql = string.Format(@"
                SELECT  Animais.IdPessoa AS IdTutor, Pessoas.Nome AS Tutor, Cardapio.IdAnimal, 
                        ...
                        Dietas.Dieta, Cardapio.EmCardapio, Cardapio.Observacao, 
                        Cardapio.Ativo, Cardapio.IdOperador, Cardapio.IP, 
                        Cardapio.DataCadastro 
                FROM    Animais INNER JOIN
                        Cardapio ON Animais.IdAnimal = Cardapio.IdAnimal INNER JOIN
                        Pessoas ON Animais.IdPessoa = Pessoas.IdPessoa LEFT OUTER JOIN
        ...
        public List<TOCardapioBll> Listar(int _usuario, int _idTutor, int _idAnimal, decimal _fator,
            int _tamPag, int _pagAtual)
        {
            ...
            string _sql = string.Format(@"
                ...
                SELECT  IdTutor, Tutor, IdAnimal, Animal, IdEspecie, Especie, Idraca, Raca,
                        ...
                        EmCardapio, Observacao, Ativo, IdOperador, IP, DataCadastro
                FROM    (
                            SELECT	ROW_NUMBER() OVER(ORDER BY Cardapio.DtCardapio DESC) AS NUMBER, 
                                    Animais.IdPessoa AS IdTutor, Pessoas.Nome AS Tutor, 
                                    Cardapio.IdAnimal, Animais.Nome AS Animal, AnimaisAuxRacas.IdEspecie, 
                                    ...
                                    Cardapio.EmCardapio, Cardapio.Observacao, Cardapio.Ativo, 
                            ...
                            FROM        AnimaisAuxEspecies INNER JOIN
                                        AnimaisAuxRacas ON AnimaisAuxEspecies.IdEspecie = 
                                            ...
                                            Animais INNER JOIN
                                        Pessoas ON Animais.IdPessoa = Pessoas.IdPessoa RIGHT OUTER JOIN
                                        Cardapio ON Animais.IdAnimal = Cardapio.IdAnimal ON 
                                            AnimaisAuxRacas.IdRaca = Animais.IdRaca LEFT OUTER JOIN
                                        Dietas ON Cardapio.IdDieta = Dietas.IdDieta
        ...
        public int TotalPaginas(int _usuario, int _idTutor, int _idAnimal, decimal _fator, int _tamPag)
        {
            CrudDal crud = new CrudDal();
            ...
            string _sql = @"
                ...
                FROM    Animais INNER JOIN
                        Pessoas ON Animais.IdPessoa = 
                            Pessoas.IdPessoa RIGHT OUTER JOIN
                        Cardapio ON Animais.IdAnimal = 
                            Cardapio.IdAnimal LEFT OUTER JOIN
                        Dietas ON Cardapio.IdDieta = Dietas.IdDieta
                WHERE   (Cardapio.Ativo = 1) ";
        ...
        public TOCardapioResumoBll CardapioResumo(int _idCardapio)
        {
            ...
            List<TOCardapioResumoBll> resumoLista =
                cardapioAlimentosBll.ListarResumoCardapio(_idCardapio);
