...
namespace BLL
{
	public class clCardapioAlimentosBll
	{
		...
		public List<TOCardapioAlimentosBll> ListarTO(int _idCard, int _idPessoa)
		{
			CrudDal crud = new CrudDal();

			string _sql = string.Format($@"
				Execute ListagemAlimentosCardapio {_idCard}, {_idPessoa}");
		...
		public List<TOCardapioResumoBll> ListarResumoCardapio(int _idCard, int _idPessoa)
		{
			CrudDal crud = new CrudDal();
			clAlimentosAuxAliasBll aliasBll = new clAlimentosAuxAliasBll();
			AlimentosAuxAlia aliasDcl;
			List<TOCardapioResumoBll> _retList = new List<TOCardapioResumoBll>();
			TOCardapioResumoBll _retItem;

            string _sql = string.Format(@"
				Execute ListarResumoCardapio {0}", _idCard);

			var lista = crud.Listar<TOCardapioResumoBll>(_sql);

			foreach (var item in lista)
			{
				aliasDcl = aliasBll.Carregar(_idPessoa, item.IdAlimento);
                _retItem = item;

                if ((aliasDcl != null) && (aliasDcl.IdAlimento > 0))
				{	
					_retItem.Alimento = aliasDcl.Alias;
                }

                _retList.Add(_retItem);
            }
		...
		public List<BICardapioAlimentosBll> BICardapioAlimento(int _idCard)
		{
			...
			string _sql = string.Format(@"
				Execute BICardapioAlimento {0}", _idCard);
        ...
        public bllRetorno AtualizarValoresComposicao(decimal _nemOriginal,
            decimal _nemcopia, int _idCardapio)
        {
            CrudDal crud = new CrudDal();
			bllRetorno _retorno = new bllRetorno();

            string _sql = string.Format(CultureInfo.CreateSpecificCulture("en-US"), 
				"Execute AtualizarValoresComposicao {0}, {1}, {2}", 
				_nemOriginal, _nemcopia, _idCardapio);

			_retorno = crud.ExecutarComando<bllRetorno>(_sql).FirstOrDefault();
