	...
	public class clPessoasBll
	{
		...
		public Pessoa Carregar(int _id)
		{
			...
			string _sql = string.Format(
                @"	SELECT	IdPessoa, IdTpPessoa, Nome, dTpEntidade, dNacionalidade, 
							DataNascimento, RG, CPF, CNPJ, Passaporte, DocumentosOutros, 
							CRVM, Email, Telefone, Celular, Logotipo, Assinatura, 
							Logradouro, Logr_Nr, Logr_Compl, Logr_Bairro, Logr_CEP, 
							Logr_Cidade, Logr_UF, Logr_Pais, Latitude, Longitude, 
							Usuario, Senha, Bloqueado, Ativo, IdOperador, IP, 
							DataCadastro, Versao
					FROM	Pessoas
        ...
        public TOPessoasBll CarregarCPF(string _cpf)
        {
            CrudDal crud = new CrudDal();
            TOPessoasBll userTO = new TOPessoasBll();
            ...
            string _sql = string.Format(@"
				...
				From	Pessoas
				Where	(Ativo = 1) And (Bloqueado is not null) AND 
						((CPF = '{0}') OR (Passaporte = '{0}') OR 
						 (CNPJ = '{0}') OR (DocumentosOutros = '{0}')) AND 
						(IdTpPessoa in (1, 2))", _cpf);
        ...
        public Pessoa CarregarTutor(Pessoa _pessoa)
        {
            CrudDal crud = new CrudDal();
            string _sql = string.Format($@"
                SET DATEFORMAT dmy                
                ...
                WHERE	(IdTpPessoa = 3) AND
                        ...
                        (IdPessoa not in (Select	IdTutor
						                  From		Tutores
            ...
            Pessoa reg = crud.ExecutarComando<Pessoa>(_sql).FirstOrDefault();
        ...
        public bool CamposPrincipaisCadastrados(int _idPessoa)
        {
            bool retorno = true;
			Pessoa _pessoa = Carregar(_idPessoa);

			if (_pessoa != null)
			{
				if ((Funcoes.Funcoes.ConvertePara.Int(_pessoa.dTpEntidade) <= 0) ||
                    (Funcoes.Funcoes.ConvertePara.Int(_pessoa.dNacionalidade) <= 0))
				{
                    retorno = false;
                }
				else
				{
					switch (_pessoa.dTpEntidade)
					{
						case (int)DominiosBll.PessoasEntidadesAuxTipos.Pessoa_Física:
							{
								switch (_pessoa.dNacionalidade)
								{
                                    case (int)DominiosBll.PessoasAuxNacionalidade.Brasileira:
										{
											if ((_pessoa.CPF == null) || (_pessoa.CPF == ""))
											{
                                                retorno = false;
                                            }
											else if ((_pessoa.Logr_CEP == null) ||
                                                     (_pessoa.Logr_CEP == ""))
                                            {
                                                retorno = false;
                                            }
                                    ...
                                    case (int)DominiosBll.PessoasAuxNacionalidade.Estrangeira:
                                        {
                                            if ((_pessoa.Passaporte == null) ||
                                                (_pessoa.Passaporte == ""))
                                            {
                                                retorno = false;
                                            }
                        ...
                        case (int)DominiosBll.PessoasEntidadesAuxTipos.Pessoa_Jurídica:
                            {
                                switch (_pessoa.dNacionalidade)
                                {
                                    case (int)DominiosBll.PessoasAuxNacionalidade.Brasileira:
