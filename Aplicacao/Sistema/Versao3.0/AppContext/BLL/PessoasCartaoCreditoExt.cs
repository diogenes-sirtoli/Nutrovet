...
namespace BLL
{
	public static class clPessoasCartaoCreditoExt
	{
			...
			bool _insersao)
		{
			...
			if (RegistroDuplicado(_cartao, false))
			{
		...
		private static bool EfetuaUpdate(PessoasCartaoCredito _cartao)
		{
			...
			int _upd = crud.ExecutarComando(string.Format(@"
				...
				Funcoes.Funcoes.Seguranca.Criptografar(_cartao.NomeCartao),
                Funcoes.Funcoes.Seguranca.Criptografar(_cartao.CodSeg),
                Funcoes.Funcoes.Seguranca.Criptografar(_cartao.VencimCartao)));
