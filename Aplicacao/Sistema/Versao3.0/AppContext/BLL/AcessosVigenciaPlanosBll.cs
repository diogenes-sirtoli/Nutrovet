...
namespace BLL
{
	public class clAcessosVigenciaPlanosBll
	{
			...
			if (_pesqNome != "")
			{
				_sql += string.Format(@" AND 
									((p.Nome Like '%{0}%'  COLLATE Latin1_General_CI_AI) OR
									 (p.Email Like '%{0}%'  COLLATE Latin1_General_CI_AI) OR
									 (p.Usuario Like '%{0}%'  COLLATE Latin1_General_CI_AI))",
					   _pesqNome);
			}
		...
		public bllRetorno EhClienteVigente(string _cpf)
		{
			...
			pessoaTO = pessoaBll.CarregarCPF(_cpf);
		...
		public bllRetorno JaExisteNomeOuEmail(string _nome, string _email)
		{
			bllRetorno bllRetorno = new bllRetorno
			{
				retorno = true,
				mensagem = "Prosseguir na Compra"
			};
			...
			if (pessoaTO != null)
			{
				if (Funcoes.Funcoes.ConvertePara.Bool(pessoaTO.cBloqueado))
				{
					if (pessoaTO.cIdTpPessoa == (int)DominiosBll.PessoasAuxTipos.Cliente)
					{
						bllRetorno.retorno = false;
						bllRetorno.mensagem =
							$"O nome: {_nome} ou e-mail: {_email} já existe em nosso cadastro, mas está com o perfil BLOQUEADO. Contate o Administrador Através do WhatsApp (61) 98136-6230!!!";
					}
				}
				if (pessoaTO.cIdTpPessoa == (int)DominiosBll.PessoasAuxTipos.Cliente)
				{
					bllRetorno.retorno = false;
					bllRetorno.mensagem =
					 $"O nome: {_nome} ou e-mail: {_email} já existe em nosso cadastro.  Faça LOGIN e Renove seu Plano Através da TELA do seu PERFIL.";
				}
			}
