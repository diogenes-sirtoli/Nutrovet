...
namespace Nutrovet.Plano
{
    public partial class FinalizarAssinatura : System.Web.UI.Page
    {
                ...
                case "NUTROVET-SERVER":
                    {
                        ...
                        // Após
                        PagarMeService.DefaultEncryptionKey = "ek_live_2xSZkpu0OMnFvmavVWY2oDTQ2eyqqf";
                        PagarMeService.DefaultApiKey = "ak_live_RB50JKyd9qa4opTBAi4N3xYZ2g61X7";
                        _servidoLocal = false;
            ...
            else
            {
                ...
                if (retornoDados.retorno)
                {
                    ...
                    if (cartaoCreditoDcl.IdCartao > 0)
                    {
                        ...
                        if (retAlteraPlanoVigente.retorno)
                        {
                            retornoDados.mensagem += (retAlteraPlanoVigente.retorno ?
                            "Cartão Adicionado no Plano do Cliente com Sucesso!</br>" :
                            "Erro Cupom: " + retAlteraPlanoVigente.mensagem + "</br>");
                        }
                    }
                    ...
                    if (retornoTutores.retorno)
                    {
                        retornoDados.mensagem += (retornoTutores.retorno ?
                            "Tutor cadastrado com Sucesso!</br>" :
                            "Erro Cupom: " + retornoTutores.mensagem + "</br>");
                    }

                    LogInsercao(pessoaDcl, retornoDados.mensagem, retornoDados.retorno);
                }
                else
                {
                    LogInsercao(pessoaDcl, retornoDados.mensagem, retornoDados.retorno);
                    ...
                    Documents = new[]
                    {
                         new Document
                         {
                             ...
                             Type = (_dadosComprador.dNacionalidade == 2 ? DocumentType.Passport :
                                        (_dadosComprador.dTpEntidade == 1 ? DocumentType.Cpf : DocumentType.Cnpj)),
                             Number = (_dadosComprador.dNacionalidade == 2 ? _dadosComprador.Passaporte :
        ...
        private void LogInsercao(Pessoa _cliente, string _msgBD, bool _sucesso)
        {
            logsDcl = new LogsSistema();

            logsDcl.IdPessoa = (_cliente != null && _cliente.IdPessoa > 0 ?
                _cliente.IdPessoa : 0);
            logsDcl.IdTabela = (int)DominiosBll.LogTabelas.Pessoas;
            logsDcl.IdAcao = (int)DominiosBll.AcoesCrud.Assinatura_de_Planos;
            logsDcl.Justificativa = "";
            logsDcl.Mensagem = string.Format(
                "Assinante {0} {3} {4}, em {1}, " +
                "{5}: {2}",
                    _cliente.Nome, DateTime.Today.ToString("dd/MM/yyyy"), _msgBD,
                    (_sucesso ? "Efetuou" : "Tentou"), 
                    (_sucesso ? "Assinatura" : "Assinar"),
                    (_sucesso ? "com sucesso, sendo" : 
                    "mas houve erro com a seguinte mensagem"));
